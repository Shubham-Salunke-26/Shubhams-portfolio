...
import { faBriefcase, faWrench, faRocket, faPenToSquare, faGraduationCap, faStar } from "@fortawesome/free-solid-svg-icons";
...
export default function GlassHeader() {
  ...
  const getIcon = (item: string) => {
    switch (item) {
      ...
      case "skills": return faWrench;
      case "projects": return faRocket;
      case "blogs": return faPenToSquare;
    ...
    <header className="sticky z-50 w-full backdrop-blur-md backdrop-filter bg-background/70 dark:bg-background/40 border-b border-border/40 supports-[backdrop-filter]:bg-background/60">
      <div className="container max-w-4xl mx-auto p-4 flex justify-between items-center">
        ...
        <nav className="hidden md:flex items-center space-x-6 text-sm font-medium">
          {["experience", "skills", "projects", "blogs", "education"].map(
            (item, index) => (
              <motion.a
                key={item}
                href={`#${item}`}
                ...
                whileHover={{ y: -2 }}
              >
                <FontAwesomeIcon icon={getIcon(item)} className="w-4 h-4" />
              ...
              {["experience", "skills", "projects", "blogs", "education"].map(
                (item, index) => (
                  <motion.a
                    key={item}
                    href={`#${item}`}
                    className="transition-colors hover:text-foreground/80 text-foreground/60 py-2 flex items-center gap-2"
                    onClick={toggleMenu}
